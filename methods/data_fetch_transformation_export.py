...
class GarminDataFetcher:
    def __init__(self):
        ...
        # self.today = datetime.date(2025, 1, 12) # for testing custom dates
    ...
    def activity_metrics(self, file_name="only_activities.csv"):
        ...
        exporter.export(activities, self.rows_list)

        print(f"Activities data saved to {file_name} with {len(activities)} rows.")

        return activities


    def all_data(self, file_name="all_metrics.csv"):
        ...
        exporter = CSVExporter(file_name)
        exporter.export(merged_data, self.rows_list)

        print(f"Merged data saved to {file_name} with {len(merged_data)} rows.")


        

    def fetch_metrics(self, append_existing=False, filename="only_metrics.csv"):
        if append_existing and os.path.exists(filename):
            ...
            print("Fetching metrics from registered date: ", start_date)
        ...
        exporter.export(self.data, self.rows_list)

        print(f"Garmin metrics data saved to {filename} with {len(self.data)} new rows.")
